...
exports.chatService = {
    ...
    async getOrCreateUserAdminRoom(userId, adminId) {
        try {
            // First, try to find existing chat room without order
            let chatRoom = await prisma_1.default.chatRoom.findFirst({
                where: {
                    userId,
                    adminId,
                    orderId: null
            ...
            if (!chatRoom) {
                chatRoom = await prisma_1.default.chatRoom.create({
                    data: {
                        userId,
                        adminId,
                        participants: {
            ...
            logger_1.logger.error('Error in getOrCreateUserAdminRoom:', error);
            throw error;
        }
    },
    // Get or create order-specific chat room (first time order chat)
    async getOrCreateOrderRoom(orderId, userId, adminId) {
        try {
            ...
            logger_1.logger.error('Error in getOrCreateOrderRoom:', error);
            throw error;
        }
    },
    // Send first message in a chat room
    async sendFirstMessage(chatRoomId, senderId, content, messageType = 'text') {
        try {
            ...
            await prisma_1.default.chatRoom.update({
                where: { id: chatRoomId },
            ...
            logger_1.logger.error('Error in sendFirstMessage:', error);
            throw error;
        }
    },
    ...
    // Get user chat rooms for admin dashboard
    async getUserChatRooms(adminId) {
        try {
            const chatRooms = await prisma_1.default.chatRoom.findMany({
                where: {
                    adminId,
                    orderId: null
                },
            ...
            logger_1.logger.error('Error in getUserChatRooms:', error);
            throw error;
        }
    },
    async getClientChatRooms(clientId) {
        try {
            const chatRooms = await prisma_1.default.chatRoom.findMany({
                where: {
                    userId: clientId,
    ...
    // Get order chat rooms for admin dashboard
    async getOrderChatRooms(adminId) {
        try {
            const chatRooms = await prisma_1.default.chatRoom.findMany({
                where: {
                    adminId,
            ...
            logger_1.logger.error('Error in getOrderChatRooms:', error);
            throw error;
        }
    },
    async getOrderUserChats(userId) {
        try {
            const chatRooms = await prisma_1.default.chatRoom.findMany({
                where: {
                    orderId: { not: null },
    ...
    async getUserChats(userId) {
        try {
            ...
            logger_1.logger.error('Error in getUserChats:', error);
            throw error;
        }
    },
    // Get chat room by ID
    async getChatRoom(chatRoomId) {
        try {
            const chatRoom = await prisma_1.default.chatRoom.findUnique({
                where: { id: chatRoomId },
                include: {
                    participants: {
                        include: {
            ...
            logger_1.logger.error('Error in getChatRoom:', error);
            throw error;
        }
    },
    // Check if user can access chat room
    async canUserAccessChat(userId, chatRoomId) {
        try {
            const chatRoom = await prisma_1.default.chatRoom.findFirst({
                where: {
                    id: chatRoomId,
                    OR: [
                        { userId },
            ...
            logger_1.logger.error('Error in canUserAccessChat:', error);
            return false;
        }
    }
};
